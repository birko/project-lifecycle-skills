...
consumes them for the drift audit. Skip this step silently when the map is absent.
...
- **Stdout-only — never writes `docs/ROADMAP.md` or any persisted status mirror.** `/roadmap` is invoked on demand, not auto-chained, so any file it committed would silently lag the source trees the moment they change — false confidence, and a committed "Divergences: none" becomes a lie. Ephemeral-by-design is correct for an audit tool. The *persisted*, always-current drift signal already exists: [[tasks]] `triage` (which IS auto-chained on every task change) writes a `⚠ Feature drift` callout into `tasks/README.md`. Don't add a second generated mirror. (A static `docs/ROADMAP.md`, if a project keeps one, is a hand-maintained, **status-free** brief→feature coverage map — not this skill's output.)
