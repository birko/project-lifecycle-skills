...
- **Changelog:** `CHANGELOG.md` — what shipped, for humans (Keep a Changelog); updated via `/roll-changelog` at ship moments (a feature reaching `done`, user-facing feature-less work)

## How we work — feature lifecycle

Real work flows through a repeatable lifecycle so it stays **tracked, testable, reviewable, and visible to stakeholders** ({{STAKEHOLDERS}}). Don't jump straight to code for anything non-trivial.
...
4. **Decompose into small tasks** — `/feature decompose` → `/tasks new`. Approved/changed decisions become atomic, independently-completable tasks under `tasks/`. Each task links back via `feature: FEATURE-NNN`. **This step comes before writing implementation code, always** — see the task-first gate under Working rules.
4b. **Entering an existing feature** — `/feature pick FEATURE-NNN`. It resolves what stage the feature is really at and offers the verb that unblocks it: undecided rows → `/feature decide`; **approved rows with no tasks → `/feature decompose`**; all tasks done → `/feature review`. It hands off to `/tasks pick --feature FEATURE-NNN` only once real tasks exist — you never start implementing straight out of a feature folder.
...
6. **Stakeholder status** — `/feature status` regenerates a plain-language rollup for {{STAKEHOLDERS}}: decision counts, build progress, what's testable, where it stands.
