...
- Chain the [[tasks]] skill's **`init` verb** (`/tasks init mode=<resolved mode> [repo=owner/name | project=KEY]`) to create `tasks/.config.yml` and the initial `tasks/README.md` dashboard. Pass the mode from intake so `init` doesn't re-ask. Don't hand-roll these files — let `tasks` own its file shapes so they stay in sync with that skill.
