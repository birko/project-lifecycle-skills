...
tracked-files-at-scan: 3884             # tree size when the scan ran — NOT a coverage numerator
coverage-drift: []                      # paths that were behavioural and unmapped; [] = none
areas:
...
- **`coverage:` and its two companions record whether the map was ever checked, how big the check was, and what real blindness it found.** Written by every [init](verbs/init.md) run; all three are overwritten each time, so a repo whose discovery is later fixed clears a stale verdict by itself.

  | Key | Value |
  |---|---|
  | `coverage` | `verified` · `not-applicable` (no behavioral code) · `unverified` |
  | `tracked-files-at-scan` | how many files git tracked when the scan ran |
  | `coverage-drift` | **a list of paths** — the files unmapped at first scan that turned out to be *behavioral*, i.e. folded into a capability area rather than into `ignore`. Empty list = none found. Absent = not computed |

  **`coverage` describes the map as written — after step 4's own reconciliation, not before.** That has to be said, because step 4 both computes the verdict and fixes what it finds, and the two readings disagree: measured across four consumer maps, the pre-reconciliation reading calls all four `unverified` and the post reading calls all four `verified`. Recording the drift is what makes the choice safe rather than lossy — the earlier moment is not thrown away, it is the list.

  **`coverage-drift` is a list of paths and never a bare count, because the classification behind it is a judgement.** Deciding a file is behavior rather than housekeeping is exactly the call the surrounding rules cannot make for you — measured on `Presenter`, an `appsettings.json` carrying `Fetch.*`, `Session.*` and `Database.*` settings consumed by three existing areas is defensibly either. **A bare number states that judgement with the confidence of a measurement and leaves no trace of what was judged**, so a later reader cannot audit it or disagree with it. The paths make it inspectable; the count is just their length.
  - This is the same failure the raw unmapped count has, one level up, and it is why that count is not stored at all: `Symbio` 33 unmapped with **no** drift and `Latent` 42 with none are pure tidying, while `WorkoutTracker`'s 14 included **six real source files** — `ReplacedPeriod.cs`, `RestPrescription.cs`, `active-plan-mirror.ts`, `session-day-mirror.ts`, `plans-segments.ts`, `vh-debug.ts` — that had shipped since the map was last touched and were invisible to every regen in between. Ranked by raw count those come out backwards. Ranked by the drift list they do not, *and* you can see which files earned the ranking.

  **`tracked-files-at-scan` is the size of the tree, not a coverage numerator.** Named at length on purpose: a `coverage-`prefixed count sitting beside `coverage: verified` reads as *"147 of 147 covered"*, which is not what it says and not a number this file carries at all — the verdict is categorical, and there is no percentage anywhere in the schema. What the size is for is re-running the check by hand later: if the tree is still that size, the scan can be reproduced.

  **An absent key is unknown, never good news.** A map with no `coverage` predates the field and reads `unverified`; missing companions read as *not computed*, not as zero or empty — the same reading `regen` gives a missing `shaped-by-derived`. Keys rather than comments because a consumer must be able to branch on them — **not** because comments are disposable: [init](verbs/init.md) step 6 forbids the re-render that would drop the map's hand-written rationale, and an existing map is edited in place. Distinct from an empty `areas:` list, which is already treated as *absent* everywhere: these exist for a map that looks populated while nothing checked it.
