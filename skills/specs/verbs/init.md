...
6. **Write** `docs/specs/.map.yml` from [templates/map.yml](../templates/map.yml) with the blessed areas, and **always write step 4's three coverage keys** beside `areas:` — `coverage:`, `tracked-files-at-scan:` and `coverage-drift:` (a list of paths, `[]` when none).
