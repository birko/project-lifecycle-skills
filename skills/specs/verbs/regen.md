...
5a. **Derive `shaped-by` — every regen, not only the flagged ones.** Union of three inputs, append-only:
   ...
   **A declared `pr:` outranks the state field; an inferred commit does not.** Where the task declares its reference, the declaration *is* the evidence — verify it (next paragraph) and, if the state disagrees with a reference that landed, **report the contradiction rather than discarding the strongest link in the trail**. Where there is no declaration and the evidence is inferred from a commit message, the state has to corroborate it: a task whose state says the work never landed contributes nothing, because a commit naming it is then a mention by construction. **Resolve which states those are from the vocabulary [[tasks]] owns** — apply the test, never a second copy of the list, which would go quietly wrong the day that vocabulary gains a state. Today it resolves to `todo` (never started) and `cancelled` (abandoned); **`in-progress` passes** — work under way has landed whatever it has committed, and reachability is what proves that, not the label. **Read the field with an anchored pattern — `^status:` — and take the value before any `#`.** Two comments sit near that field and each defeats a different sloppy read, so the pattern is given rather than the intent:
     - a **trailing** comment on the field itself — a real task file writes `status: done  # merged 5414637e; 6 unit tests`, and a naive *line* match rejects every one of them;
     - a **leading** comment one line above, which the task template emits on every file: `# status: todo | in-progress | review (code done, sign-off pending) | blocked | done | cancelled`. It contains **every legal value**, so an unanchored first match returns the comment and parses as `todo` — concluding the work never landed, for *every* template-generated task including the `done` ones. Anchoring to column 0 is **not** sufficient on its own if you scan the whole file, because a fenced example can sit at column 0 too; scope the read to the frontmatter between the first two `---` fences.
     - **The hazard is symmetric, and the write side is worse.** Measured 2026-08-23: a `/fix-next` run set a task to `in-progress` with an unanchored substring replace, hit the enum comment instead of the field, and ran the task to completion — fix, verification, a full `## Outcome` — while its frontmatter still read `todo`. A bad read yields a wrong answer; a bad write silently succeeds against the wrong line and leaves the record and the work claiming different states.

     A `status:` that is missing or unreadable fails the test — count the task unresolved rather than assuming either way.
   ...
   **Do not narrow the inferred-evidence gate to `done` alone.** Measured on a real project, `done`-only drops **27 of 96** attributions that are subject-authored and already in the tree — tasks parked at `review` (merged, manual sign-off open) and `blocked` (merge deferred) — while removing no false positive that the subject rule below had not already removed. Measured the other way: **10** attributions come from `todo`/`cancelled` tasks that nonetheless lead a commit subject. That is a self-contradicting trail, not evidence — count it unresolved and say so in step 7, rather than silently believing one side over the other.
   ...
   - **`pr:` is null** → fall back to commit messages, but read **only the subject line**: `git log --format='%H%x1f%s' --grep '\bTASK-NNN\b'` — the `--grep` is a cheap prefilter over the whole message, and the `\b` boundaries stay, because without them `TASK-11` also matches `TASK-110`…`TASK-119` (measured: 22 false-positive commits vs 0 bounded). Then keep a commit **only when the first task id in its subject is this task's**, and run the same `git show` form on what survives.
     ...
     - A project whose commits never name the task in a subject resolves nothing here. That is the honest answer — it raises `shaped-by-unresolved` (step 5b) instead of inventing provenance. The durable fix is upstream, not a looser match: [[tasks]] `close` writes the commit SHA into `pr:` — **on `pr-per-task` projects**, where the merge commit can carry an edit naming the work commit. It is **not** retroactive — measured on a real project, a run after it shipped still fell back here for 120 of 213 feature-linked tasks — so expect this path to carry old history for a long time.
     - **A `single-branch` project reaches this path on every task, permanently, and that is by design.** There is no second commit to carry the reference and a commit cannot contain its own hash, so `close` deliberately leaves `pr:` null there ([close.md](../../tasks/verbs/close.md) step 7). **So on those repos the subject rule is not a fallback — it is the only provenance path there is.** Do not read a null `pr:` on a `single-branch` repo as a task that skipped its close; read `integration:` in `tasks/.config.yml` before drawing any conclusion from an empty field.
